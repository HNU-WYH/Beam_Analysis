...
def simple_support_beam(load_type, solv_type = SolvType.STATIC):
    ...
    if load_type == LoadType.F and solv_type == SolvType.STATIC:
        ...
        plt.title('Comparison of Theoretical and Numerical Solutions for Simple Supported Beam Displacement')
        plt.legend()
        plt.grid(True)
        ...
        for error, color, label in zip(errors, colors[:-1], labels[:-1]):
            plt.plot(x_values, error, color=color, label=f'Error (num_elements={label.split("=")[-1]})')

        plt.xlabel('Position along the beam (x) [m]')
        plt.ylabel('Error(mm)')
        # plt.yscale('log')
        plt.title('Error between Numerical and Theoretical Solutions for Simple Supported Beam Displacement')
        plt.legend()
        plt.grid(True)
        plt.show()
        ...
        for num_elements in num_list:
            ...
            # Add constraints: displacement at node 0 is 0, displacement at node -1 (end) is 0
            fem.add_constraint(0, 0, ConstraintType.DISPLACEMENT)
            fem.add_constraint(fem.beam.num_nodes-1, 0, ConstraintType.DISPLACEMENT)
        ...
        # calculate theoretical displacement for uniformly distributed load
        def dis(x, q = 100, L = 5):
            return -(q / (2 * E * I)) * ((L / 6) * x**3 - (x**4) / 12) + (q * L**3 * x) / (24 * E * I)
        ...
        for tau in [0.001]:
            for num_elements in [1, 5, 50]:
                ...
                temp = FEM(temp)
                temp.apply_force((2.5, 500), LoadType.F)
                temp.add_constraint(0, 0, ConstraintType.DISPLACEMENT)
                temp.add_constraint(fem.beam.num_nodes-1, 0, ConstraintType.DISPLACEMENT)
                temp.solv()
