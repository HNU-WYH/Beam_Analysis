...
class FrameworkFEM:
    ...
    def visualize(self, sol_type=SolvType.STATIC, title: str = 'Deformed Structure'):
        ...
        if sol_type == SolvType.STATIC:
            ...
            plt.scatter(x, y, c='b', s=5, label='Original Beam')
            plt.scatter(x_st, y_st, c='r', s=5, label='Deformed Beam')
            plt.xlabel('x (m)')
            plt.ylabel('y (m)')
            plt.title(title)
            plt.legend()
            ...
            fig, ax = plt.subplots()
            line, = ax.plot([], [], 'r.', markersize=5, label='Dynamic Solution')
            ax.scatter(x, y, c='b', s=5, label='Original Beam')
